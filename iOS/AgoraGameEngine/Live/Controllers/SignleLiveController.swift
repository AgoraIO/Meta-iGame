...
class SignleLiveController: BaseViewController {
    public lazy var liveView: LiveBaseView = {
        let view = LiveBaseView(channelName: channleName, currentUserId: currentUserId, roomName: roomName)
        return view
    }()
    public var agoraKit: AgoraRtcEngineKit?
    private lazy var rtcEngineConfig: AgoraRtcEngineConfig = {
        let config = AgoraRtcEngineConfig()
        config.appId = KeyCenter.rtc_app_id
    ...
    init(channelName: String, sceneType: SceneType, userId: String, roomName: String = "", agoraKit: AgoraRtcEngineKit? = nil) {
    ...
    public func joinBroadcasterChannel(channelName: String, uid: UInt) {
        ...
        let result = agoraKit?.joinChannel(byToken: nil, channelId: channelName, uid: uid, mediaOptions: channelMediaOptions, joinSuccess: nil)
        if result == 0 {
    ...
    public func joinAudienceChannel(channelName: String, pkUid: UInt = 0) {
        ...
        let joinResult = agoraKit?.joinChannelEx(byToken: nil, connection: connection, delegate: self, mediaOptions: channelMediaOptions, joinSuccess: nil) ?? -1000
        if joinResult == 0 {
