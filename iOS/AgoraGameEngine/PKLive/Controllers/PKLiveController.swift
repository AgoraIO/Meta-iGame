...
class PKLiveController: SignleLiveController {
    ...
    public func pkSubscribeHandler(object: IObject) {
        guard var model = JSONObject.toModel(PKApplyInfoModel.self, value: object.toJson()) else {
            return
        }
        guard pkApplyInfoModel != model else {
            return
        }
        ...
        if model.status == .end {
            ...
            let message = sceneType == .game ? String(format: "your_friends_invite_you_join_game".localized, model.userName, "王国激战") : ""
