...
class GameCenterView: UIView {
    ...
    private func getGameList() {
        vm.fetchNextPagFameList(sceneType: sceneType) { [weak self](list) in
            guard let list = list else { return }
            self?.tempList.append(contentsOf: list)
            self?.dataArray = self?.tempList
        } all: { [weak self](list) in
            guard let list = list else { return }
            self?.tempList = list
            self?.dataArray = self?.tempList
        }
...
extension GameCenterView: AGECollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row + 1 == (collectionLayout.dataArray?.count ?? 0) {
            getGameList()
        }
        return cell
    }
