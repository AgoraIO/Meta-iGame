...
class GameLiveController: PKLiveController {
    ...
    override func viewDidLoad() {
        self.webView = GameWebView(viewModel: viewModel)
        super.viewDidLoad()
        setupUI()
    ...
    private func gameApplySubscibeHandler(object: IObject) {
        guard let model = JSONObject.toModel(GameApplyInfoModel.self, value: object.toJson()) else {
            return
        }
        ...
        gameInfoModel.vendorId = pkApplyInfoModel?.vendorId ?? ""
...
//            view.layer.contents = model.gameId.bgImage?.cgImage
    ...
    private func clickGamePKHandler() {
        let gameCenterView = GameCenterView(sceneType: .game, vm: viewModel)
        gameCenterView.didGameCenterItemClosure = { [weak self] gameCenterModel in
            self?.gameCenterModel = gameCenterModel
            guard let vendorId = gameCenterModel.vendorId else {
                fatalError("no vendorId")
            }
            self?.inviteBroadcastHandler(venderId: vendorId)
        }
        AlertManager.show(view: gameCenterView, alertPostion: .bottom)
    }
    
    private func inviteBroadcastHandler(venderId: String) {
        let pkInviteListView = PKLiveInviteView(channelName: channleName, sceneType: sceneType)
        pkInviteListView.venderId = venderId
...
//        liveView.playGifView.isHidden = getRole(uid: UserInfo.uid) == .broadcaster && pkInfoModel?.status == .accept
    ...
    private func updatePKUIStatus(isStart: Bool, isAudience: Bool = false) {
        LogUtils.log(message: "isStart \(isStart)", level: .info)
        ...
        if isStart {
            ...
            guard let vendorId = gameInfoModel?.vendorId else {
                fatalError()
            }
    ...
    deinit {
        LogUtils.log(message: "GameLiveController deinit", level: .info)
    }
}


extension GameLiveController: GameViewModelDelegate {
    func gameViewModelDidRecv(msg: String) {
        liveView.sendMessage(message: msg, messageType: .message)
    }
}
