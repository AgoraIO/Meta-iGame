...
class SyncUtil: NSObject {
    ...
    static func initSyncManager(sceneId: String) {
        let config = AgoraSyncManager.RtmConfig(appId: KeyCenter.rtc_app_id, channelName: sceneId)
    ...
    class func update(id: String,
                      ...
                      fail: FailBlock? = nil) {
        LogUtils.log(message: "=== update \(id) \(key ?? "") \(params)", level: .info)
        let sceneRef = sceneRefs[id]
    ...
    class func subscribe(id: String,
                         ...
                         fail: FailBlock? = nil) {
        let sceneRef = sceneRefs[id]
        LogUtils.log(message: "=== subscribe \(id) \(key ?? "")", level: .info)
