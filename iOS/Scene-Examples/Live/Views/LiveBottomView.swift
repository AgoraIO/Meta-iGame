...
class LiveBottomView: UIView {
    ...
    private func setupUI() {
        chatButton.backgroundColor = UIColor(hex: "#000000", alpha: 0.6)
        chatButton.setTitle("Live_Text_Input_Placeholder".localized, for: .normal)
        chatButton.setTitleColor(.white, for: .normal)
        chatButton.titleLabel?.font = .systemFont(ofSize: 14.fit)
        chatButton.layer.cornerRadius = 19
        chatButton.layer.masksToBounds = true
        chatButton.setContentHuggingPriority(.defaultLow, for: .horizontal)
        chatButton.setContentCompressionResistancePriority(.defaultLow, for: .horizontal)
...
//            chatButton.configuration = conf
            chatButton.titleEdgeInsets = UIEdgeInsets(top: 0, left: 15, bottom: 0, right: 0)
        } else {
            chatButton.titleEdgeInsets = UIEdgeInsets(top: 0, left: 15, bottom: 0, right: 0)
        }
